...
public class DetailServiceImpl implements DetailService {
	...
	//根据id获取区域信息
	@Override
	public Beat getBeatByIDS(Integer beat_id) {
		// TODO Auto-generated method stub
		return detailDao.getBeatByIdS(beat_id);
	}
	//根据区域id获取攻略列表
	@Override
	public Page getStrateByBeat_id(Integer beat_id,Integer index) {
		...
		p.setIndex(index);
		p.setSize(10);
		p.setCount(detailDao.getBeatStrateCount(beat_id));
		p.setPageCount((int)Math.ceil((double)p.getCount()/p.getSize()));
		Map<String, Object> map=new HashMap<String, Object>();
		map.put("size", p.getSize());
		map.put("beat_id", beat_id);
		map.put("index", (p.getIndex()-1)*p.getSize());
		p.setObj(detailDao.getStrateListByBeat_id(map));
		return p;
	}
	//根据攻略id获取攻略
	@Override
	public Strate getStrateBySID(Integer strate_id) {
		detailDao.addStrateLeadNum(strate_id);
		return detailDao.getStrateBySID(strate_id);
	}
