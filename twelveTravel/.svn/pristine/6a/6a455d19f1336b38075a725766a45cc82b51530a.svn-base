...
<body>
	...
	<script type='text/javascript'>
		...
		function getdata(index) {
			...
			$.ajax({
						url : "/twelveTravel/strate/getsclist",
						data : content,
						type : "post",
						dataType : "json",
						success : function(data) {
							...
							$(data.obj).each(
											function() {
												...
												html += "<td>"+ timeConverter(this.time)+ "</td>";
												html += "<td title='"+this.comment_txt+"'>"+(this.comment_txt.length >8?this.comment_txt.substring(0,8)+"...":this.comment_txt)+"</td>";
												html += "<td style='width:191px'><a id='del' href='/twelveTravel/strate/detelescomment?comment_id="+ this.comment_id
